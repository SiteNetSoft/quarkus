...
==== Reloading the browser in development mode

In development mode, a changed static resource is served on the next request, but the browser has to be refreshed to see it.
Setting `quarkus.http.browser-live-reload=true` makes Quarkus notify the browsers showing a page of the application when a
static resource changes, or when any other file that does not require a restart changes, such as a template: a changed
style sheet is applied in place, anything else reloads the page. The page has to include the script served by Quarkus:

[source,html]
----
<script src="/q/browser-live-reload.js"></script>
----

The setting has no effect outside of development mode, and the script is not served in a packaged application, so the tag can stay in the page.
